        ...
        this.server = tls.createServer({
            ...
            const connection = {
                ...
                write: data => {
                    if (!socket.destroyed) {
                        socket.write(data);
                        console.log(`${this.name}: Data sent to ${connection.id}. (size: ${data.length}) %c${data}`, 'color: dodgerblue');
                    }
                }
            };
            ...
            socket.on('data', data => { // 데이터를 받았을 때
                console.log(`${this.name}: Data received from ${connection.id}. (size: ${data.length}) %c${data}`, 'color: tomato');
