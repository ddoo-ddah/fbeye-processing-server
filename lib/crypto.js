...
function createKey(password) {
    return new Promise(async (resolve, reject) => {
        const key = (await scrypt(password, salt, settings.settings.crypto.length)).toString('base64');
        if (key) {
            resolve(key);
        } else {
...
function encrypt(data, key, encoding) {
    return new Promise((resolve, reject) => {
        const cipher = crypto.createCipheriv(settings.settings.crypto.algorithm, Buffer.from(key, 'base64'), Buffer.alloc(16, 0));
...
function decrypt(encrypted, key, encoding) {
    return new Promise((resolve, reject) => {
        const decipher = crypto.createDecipheriv(settings.settings.crypto.algorithm, Buffer.from(key, 'base64'), Buffer.alloc(16, 0));
